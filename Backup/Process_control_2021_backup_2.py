...
text2= st.text_input("1. Please input folder name for data analysis (MUST)",trial_path,key="setup_data")
path=text2 +'/'
...
def create_process_indicator(base_week): 
    ...
    return  base_week,process_indicator_df,name_list_dict,UCL,LCL,Mean

final_data,process_indicator_df,name_list_dict,UCL,LCL,Mean= create_process_indicator(final_data)
...
#@st.cache(suppress_st_warning=True,allow_output_mutation=True)
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def line_chart_by_hour(process_select):
    fig_all=[]
...
if st.checkbox('Please tick to this box if you need to show control chart'):
    fig_all=line_chart_by_hour(process_select)    
    for fig in fig_all:
        st.plotly_chart(fig)
...
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def box_chart_by_hour(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data[process_name] # process name
      i=1
...
if st.checkbox('Show box chart by hour'):
    fig_all_2=box_chart_by_hour(process_select)    
...
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def box_chart_by_day(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data[process_name] # process name
      i=1
...
if st.checkbox('Show box chart by day'):
    fig_all_2=box_chart_by_day(process_select)    
    for fig in fig_all_2:
        st.plotly_chart(fig)
...
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def box_chart_by_week(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data[process_name] # process name
      i=1
...
if st.checkbox('Show box chart by week'):
    fig_all_2=box_chart_by_week(process_select)    
    for fig in fig_all_2:
        st.plotly_chart(fig)
...
text2= st.text_input("1. Please input folder name for data analysis (MUST)",trial_path,key="applied_data")
path=text2 +'/'
#path='//Vn01w2k16v18/data/Copyroom/Test_software/Data/Membrane 3000S/'
#st.write('path input: '+path)

st.write('Accept any public folders like copy room or QA folder, currently not allow private folder')
text3=st.text_input("2. Please input folder name for saving (option)",'//Vn01w2k16v18/data/Copyroom/Test_software/Data/Save',key="applied_save")
path3=text3+'/'
...
all_files1=glob.glob(path + '*.xlsx')
all_files2=glob.glob(path + '*.xlsm')
all_files_new=all_files1+all_files2
#sort file in directory by reverse:
all_files_new = sorted(all_files_new, reverse = False)
st.text('number of files: '+str(len(all_files_new)))
st.text(all_files_new)
# Get master sheet dataframe:
@st.cache(suppress_st_warning=True)
def master_sheet_data_func2(all_files_new):
    path_name=all_files_new[-1] # get latest file
    xls = xlrd.open_workbook(path_name, on_demand=True)
    sheet_names=xls.sheet_names()
    master_sheet_name=sheet_names[0] 
    master_sheet=pd.read_excel(xls, master_sheet_name)
    return master_sheet
master_sheet_data_2=master_sheet_data_func2(all_files_new)

#---------------process data-------------------------# 
#@st.cache(suppress_st_warning=True,allow_output_mutation=True)
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def create_sheet_dict2(all_files):
  all_process_week={}
  for path_name in all_files: # Đọc 2 file đầu tiên thôi
    week_name=path_name[-16:-5]
    print(week_name,'-'*20)
...
all_process_week2=create_sheet_dict2(all_files_new) # Run function above
...
#@st.cache(suppress_st_warning=True,allow_output_mutation=True)
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def concat_baseweek2(master_sheet_data,all_process_week):
    base_week={}
    #Shallow copy: (wrong, reference to wk 37)
    #base_week=all_process_week[list(all_process_week.keys())[-1]].copy() # must have copy()
    #Deep copy:
...
final_data_2=concat_baseweek2(master_sheet_data_2,all_process_week2)    
#print(base_week['39682'])
...
#-----------create_process_indicator-------------------
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def create_process_indicator2(base_week,UCL,LCL,Mean): 
...
final_data_2,process_indicator_df_2,name_list_dict_2= create_process_indicator2(final_data_2,UCL,LCL,Mean)
...
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def highlight_process_indicator2(process_indicator_df): 
    return process_indicator_df.style.apply(hightlight_price2, axis=1)

process_indicator_df_2=highlight_process_indicator2(process_indicator_df_2)

st.text('Highlight yellow for dim below lower limit:')
st.dataframe(process_indicator_df_2)
#-----------------------Select process to show--------------------------------#
st.subheader('Select process to show:')
process_list=list(final_data_2.keys())
process_select = st.multiselect('Select process to show: ', process_list)
...
#@st.cache(suppress_st_warning=True,allow_output_mutation=True)
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def line_chart_by_hour2(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data_2[process_name] # process name
      i=1
      #Layout
      fig = make_subplots(          # Dim name
          rows=len(df_dict), cols=1,
          ...
          #vertical_spacing=0.05, # adjust spacing between charts
          #column_widths=[0.8, 0.2],
          subplot_titles=(name_list_dict_2[process_name]) # dict with key is process name and value is list of dim (contain name, cp, cpk...)
      )
      for name in list(df_dict.keys()): #also group
        df=df_dict[name].copy()
        df=df.sort_values(by=['Hour'])
...
if st.checkbox('Please tick to this box if you need to show control chart',key="box_2"):
    fig_all=line_chart_by_hour2(process_select)    
    for fig in fig_all:
        st.plotly_chart(fig)
...
st.header("Box chart by hour")
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def box_chart_by_hour2(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data_2[process_name] # process name
      i=1
...
if st.checkbox('Show box chart by hour',key="box_3"):
    fig_all_2=box_chart_by_hour2(process_select)    
    for fig in fig_all_2:
        st.plotly_chart(fig)
...
st.header("Box chart by day")
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def box_chart_by_day2(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data_2[process_name] # process name
      i=1
...
if st.checkbox('Show box chart by day',key="box_4"):
    fig_all_2=box_chart_by_day2(process_select)    
    for fig in fig_all_2:
        st.plotly_chart(fig)
...
st.header("Box chart by week")
@st.cache(suppress_st_warning=True,allow_output_mutation=True)
def box_chart_by_week2(process_select):
    fig_all=[]
    for process_name in process_select:
      df_dict=final_data_2[process_name] # process name
      i=1
...
if st.checkbox('Show box chart by week',key="box_5"):
    fig_all_2=box_chart_by_week2(process_select)    
    for fig in fig_all_2:
        st.plotly_chart(fig)
