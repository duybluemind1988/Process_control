...
```{r}
"83620-Diameter" %in% names(all_process_week[["WK6 - 2020"]])
```
      ...
      if (i==0){
        ...
        if (sheet_name %in% names(sheet_list)){
          if (dim_name %in% names(sheet_list[[sheet_name]])){
            sheet_list[[sheet_name]][[dim_name]] <- append(sheet_list[[sheet_name]][[dim_name]],dim_data)
          }
        }
      }
    }
...
```{r}
head(sheet_list[["83735-Burr and Dim"]][["Burr (bên trái/ Left)"]]$hour)
tail(sheet_list[["83735-Burr and Dim"]][["Burr (bên trái/ Left)"]]$hour)
```
